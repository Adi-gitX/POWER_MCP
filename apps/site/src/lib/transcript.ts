/**
 * The committed eval transcripts under docs/evals, parsed at build time in a
 * server component. The proof page and the home page's proof block both read
 * this, so neither can claim something a real run did not do.
 */
...
export function loadTranscript(): Transcript {
  ...
  for (const phase of ['1', '2', '3']) {
    let lines: string[];
    try {
      lines = readFileSync(join(dir, `phase-${phase}.jsonl`), 'utf8').split('\n');
    } catch {
      continue;
    }
    ...
    for (const line of lines) {
      ...
      if (e.type === 'assistant') {
        for (const c of e.message?.content ?? []) {
          ...
          if (c.type === 'tool_use' && String(c.name).startsWith('mcp__power__')) {
            pending.set(c.id, String(c.name).replace('mcp__power__', ''));
          }
        }
      }
