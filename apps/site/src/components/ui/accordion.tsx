  ...
  return (
    <AccordionPrimitive.Header className="flex">
      <AccordionPrimitive.Trigger
        className={cn(
          'flex flex-1 items-center justify-between gap-4 py-5 text-left font-medium transition-colors hover:text-blue [&[data-state=open]>svg]:rotate-180',
          className,
        )}
        {...props}
      >
...
export function AccordionContent({ className, children, ...props }: React.ComponentProps<typeof AccordionPrimitive.Content>) {
  return (
    <AccordionPrimitive.Content
      ...
      {...props}
    >
      <div className={cn('pb-5 pr-10 text-ink-2', className)}>{children}</div>
    </AccordionPrimitive.Content>
  );
}
