...
import { SITE, esc, type Page } from '../layout.js';

interface Doc { slug: string; title: string; description: string; section: string; html: string }

const nav = (docs: Doc[], current: string) => {
  const sections = [...new Set(docs.map((d) => d.section))];
  return `<aside class="toc" aria-label="Docs">${sections
    .map((s) => `<h4>${s}</h4>${docs.filter((d) => d.section === s).map((d) => `<a href="${d.slug}.html"${d.slug === current ? ' aria-current="page"' : ''}>${d.title}</a>`).join('')}`)
    .join('')}</aside>`;
};

const wrap = (docs: Doc[], d: Doc): Page => ({
  path: `docs/${d.slug}`,
  title: d.title,
  description: d.description,
  body: `<div class="wrap docs">${nav(docs, d.slug)}<article class="doc"><h1>${d.title}</h1>${d.html}</article></div>`,
});

const phases = (t: (typeof TOOLS)[number]) => (t.phases === '*' ? 'any' : t.phases.join(', '));

const toolTable = `<div class="tablewrap"><table>
<thead><tr><th>Tool</th><th>Cost</th><th>Legal in</th><th>What it does</th></tr></thead>
<tbody>${TOOLS.map(
  (t) => `<tr><td><code>${t.name}</code>${t.live_capable ? ' <span class="pill live">live</span>' : ''}${t.async ? ' <span class="pill">async</span>' : ''}</td><td>${t.metered ? '1 action' : '<span class="pill free">free</span>'}</td><td class="mono" style="font-size:.8rem">${phases(t)}</td><td>${esc(t.summary)}</td></tr>`,
...
<div class="grid2" style="margin:1.5rem 0">
  <a class="card" href="getting-started.html"><h3>Getting started →</h3><p>Connect in two minutes and build your first project.</p></a>
  <a class="card" href="how-it-works.html"><h3>How it works →</h3><p>The phases, the gates, the two human decisions.</p></a>
  <a class="card" href="tools.html"><h3>Tool reference →</h3><p>Every tool, what it costs, when it is legal. Generated from the server.</p></a>
  <a class="card" href="export.html"><h3>Export &amp; self-hosting →</h3><p>Take everything with you. It runs unchanged.</p></a>
</div>
...
<h2>Secrets</h2>
<p>Collected in the browser, stored encrypted, injected as environment variables at process start, never written to disk or git, and scrubbed from every response. See <a href="secrets.html">Secrets</a>.</p>
...
export const docs: Page[] = DOCS.map((d) => wrap(DOCS, d));
